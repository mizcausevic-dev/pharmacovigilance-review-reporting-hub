// Structure + data-integrity tests for Pharmacovigilance Review and Reporting Hub
...
test("HTML5 doctype + lang + title", () => {
  ...
  assert.match(html, /<title>Pharmacovigilance\s+Review\s+and\s+Reporting\s+Hub/);
});
...
test("4 stat tiles match synthetic data", () => {
  ...
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">18<\/span><span class="stat-label">Open\ \+\ recent\ AE\ events/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">11<\/span><span class="stat-label">Serious\ AE\ events/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">6<\/span><span class="stat-label">Due\ within\ 48h/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">2<\/span><span class="stat-label">7\-day\ expedited/);
});
...
test("First anomaly card content present (data-integrity proxy)", () => {
  assert.ok(html.includes("5 cases for Drug X in 30 days"), "First anomaly substring missing from rendered HTML");
});
...
test("First audit event kind present (data-integrity proxy)", () => {
  assert.ok(html.includes("pv.event.case-intake"), "First audit event kind missing from rendered HTML");
});
...
test("CNAME matches expected subdomain", () => {
  ...
  assert.equal(cname, "pv.kineticgain.com");
});
